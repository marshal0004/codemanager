...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const collectionsListTab = document.getElementById("collections-list"); // This is for the collections tab
  ...
  function setupEventListeners() {
    ...
    // Keyboard shortcuts
    document.addEventListener("keydown", handleKeyboardShortcuts);
    // Collections tab specific event listeners
    document
      .getElementById("create-js-collection")
      ?.addEventListener("click", () => createTemplateCollection("JavaScript"));
    document
      .getElementById("create-css-collection")
      ?.addEventListener("click", () => createTemplateCollection("CSS Styles"));
    document
      .getElementById("create-react-collection")
      ?.addEventListener("click", () =>
        createTemplateCollection("React Components")
      );
    document
      .getElementById("create-python-collection")
      ?.addEventListener("click", () =>
        createTemplateCollection("Python Scripts")
      );
    document
      .getElementById("manage-collections")
      ?.addEventListener("click", () => openDashboard());
    document
      .getElementById("search-collections")
      ?.addEventListener("input", searchCollectionsAction);
  }
        ...
        function (authResult) {
          ...
          chrome.runtime.sendMessage(
            ...
            function (response) {
              ...
              console.log(
                "🔵 Chrome runtime last error check:",
                chrome.runtime.lastError
              );
  ...
  function createTemplateCollection(templateName) {
    if (!isConnected()) {
      showError("Not connected to server. Please connect first.");
      return;
    }

    chrome.storage.local.get(["authToken", "userEmail"], function (authResult) {
      if (!authResult.authToken || !authResult.userEmail) {
        showError("Please login first to create collections");
        showLoginSection();
        return;
      ...
      chrome.runtime.sendMessage(
        ...
        function (response) {
          if (response && response.success) {
            showNotification(`${templateName} collection created!`);
            loadCollections();
            updateChangeStats();
          } else {
            showError(
              response?.error || `Failed to create ${templateName} collection`
            );
          }
        }
      );
  ...
  function searchCollectionsAction() {
    const query = document.getElementById("search-collections").value.trim();
    if (query.length > 2) {
      chrome.runtime.sendMessage(
        {
    ...
    chrome.storage.local.get(
      ...
      function (result) {
        ...
        if (result.userEmail && result.authToken && result.isAuthenticated) {
          ...
          loadCollections(); // Ensure collections load after auth is confirmed
...
function loadCollections() {
  console.log("🔍 LOADING COLLECTIONS - Starting request");

  chrome.runtime.sendMessage({ action: "getCollections" }, function (response) {
    console.log("🔍 COLLECTIONS RESPONSE:", response);

    if (response && response.collections) {
      console.log("✅ Collections received:", response.collections.length);
      displayCollections(response.collections);
    } else {
  ...
  function displayCollections(collections) {
    console.log("🔵 DISPLAY COLLECTIONS - Starting", {
      collectionsCount: collections?.length || 0,
      collectionsData: collections,
      activeTabId: document.querySelector(".tab-content.active")?.id,
    });

    // Determine which collections list to update based on active tab
    const activeTab = document.querySelector(".tab-content.active");
    const targetList =
      activeTab?.id === "collections-tab"
        ? document.querySelector("#collections-tab #collections-list")
        : collectionsList;

    if (!targetList) {
      console.error("🔴 Target collections list not found", {
        activeTabId: activeTab?.id,
        collectionsTabList: !!document.querySelector(
          "#collections-tab #collections-list"
        ),
        collectionsListExists: !!collectionsList,
      });
      return;
    }
    ...
    console.log("🔵 Target list found:", {
      ...
      targetListClass: targetList.className,
    });
    ...
    if (!collections || collections.length === 0) {
      ...
      <div style="text-align: center; color: #666; padding: 15px; font-size: 12px;">
          No collections yet
          <br><br>
          <button class="btn primary" onclick="document.getElementById('create-collection-btn').click()">
    ...
    collections.forEach((collection, index) => {
      console.log(`🔵 Rendering collection ${index + 1}:`, {
        id: collection.id,
        name: collection.name,
        snippetCount: collection.snippet_count,
      });
      ...
      if (activeTab?.id === "collections-tab") {
        ...
        <div class="collection-count">${collection.snippet_count || 0}</div>
      `;
      } else {
        collectionElement.innerHTML = `
      ...
      collectionElement.addEventListener("click", () => {
        ...
        chrome.storage.local.get(["serverUrl"], function (result) {
          if (result.serverUrl) {
            const dashboardUrl = `${result.serverUrl}/dashboard/collections/${collection.id}`;
            console.log("🔵 Opening collection dashboard:", dashboardUrl);
      ...
      targetList.appendChild(collectionElement);
    });

    console.log("✅ Collections display completed successfully");
  }
  ...
  function openDashboard() {
    chrome.storage.local.get(["serverUrl"], function (result) {
      if (result.serverUrl) {
        // Open dashboard route instead of root
        chrome.tabs.create({ url: result.serverUrl + "/dashboard/" });
      } else {
        showError("No server URL configured");
      }
    });
