...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function init() {
    ...
    setupMessageListeners(); // Add this line
  }
      ...
      async (message, sender, sendResponse) => {
        ...
        switch (message.type) {
          ...
          case "LOGIN_SUCCESS":
            ...
            // CRITICAL: Verify this is actually a success with required data
            if (!message.data || message.data.success === false) {
              console.error("🔴 False login success - data indicates failure");
              showError("Authentication failed - server rejected login");
              break;
            }
            ...
            const user = message.data.user ||
              message.data.userData || { email: emailInput.value };
            ...
            // CRITICAL: Both token and user must be present and valid
            if (token && token.length > 10 && user && user.email) {
              const authData = {
                userEmail: user.email,
                authToken: token,
              ...
              chrome.storage.local.set(authData, function () {
                if (chrome.runtime.lastError) {
                  ...
                  showError("Failed to save login session");
                  return;
                }
              ...
              console.error(
                "🔴 Invalid login success - missing required auth data:",
                {
                  hasToken: !!token,
              ...
              showError("Authentication failed - incomplete session data");
            }
            break;
          case "LOGIN_FAILED":
      ...
      chrome.runtime.sendMessage(
        ...
        function (response) {
          ...
          if (response && response.success) {
            ...
            const errorMsg =
              response?.error || "Failed to create collection - no response";
            ...
            if (
              ...
              chrome.storage.local.remove(
                ["userEmail", "authToken", "isAuthenticated"],
                function () {
                  showLoginSection();
    ...
    chrome.runtime.sendMessage(
      ...
      function (response) {
        ...
        console.log("🔵 Login response received:", {
          ...
          hasToken: !!(response?.token || response?.authToken),
          hasUser: !!(response?.user || response?.userData),
          errorMsg: response?.error || response?.message,
        });

        // CRITICAL: Only proceed if response exists AND success is explicitly true
        if (response && response.success === true) {
          const token =
            ...
            response.jwt;
          const user = response.user || response.userData || { email: email };

          if (token && token.length > 10) {
            // Token should be substantial
            console.log("🔵 Valid login - storing auth data:", {
              email: email,
            ...
            console.error("🔴 Login success but invalid/missing token");
            showError("Authentication failed - invalid session token");
          }
        } else {
          ...
          if (response) {
            if (response.error) {
              ...
              // Parse server messages for specific errors
  ...
  function validateAuthResponse(response) {
    console.log("🔍 Validating auth response:", response);

    // Must have response object
    if (!response) {
      return { valid: false, error: "No response from server" };
    }

    // Must explicitly be successful
    ...
    // Must have user data or be able to construct it
    const user = response.user || response.userData;
    if (!user || !user.email) {
      return { valid: false, error: "Missing user information" };
    }
  ...
  function registerUser() {
    ...
    chrome.runtime.sendMessage(
      ...
      function (response) {
        ...
        // Use the same validation logic as login
        const validation = validateAuthResponse(response);

        if (validation.valid) {
          console.log("🔵 Valid registration - storing auth data");
          ...
          chrome.storage.local.set(
            ...
            function () {
              if (chrome.runtime.lastError) {
                console.error(
                  "🔴 Failed to store registration auth data:",
                  chrome.runtime.lastError
                );
                showError("Failed to save registration session");
                return;
              }
  ...
  function showError(message, context = "general") {
    ...
    showNotification(displayMessage, errorType);
